...
export default function AchievementCard({ achievement, index }: AchievementCardProps) {
  const handleOpen = () => {
    if (achievement.proofLink) {
      window.open(achievement.proofLink, '_blank', 'noopener,noreferrer');
    }
  };
  ...
  return (
    <motion.div
      ...
      className={`group relative ${achievement.proofLink ? 'cursor-pointer' : ''}`}
      onClick={achievement.proofLink ? handleOpen : undefined}
    >
